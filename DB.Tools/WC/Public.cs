        ...
        foreach (LINAA.SubSamplesRow s in samples)
        {
            ...
            foreach (string ele in elementsIfAny)
            {
                ...
                foreach (LINAA.NAARow n in naas)
                {
                    ...
                    LINAA.SigmasRow sigma = ir.NAARow.SigmasRowParent;
                    ...
                    ir.Asp = (s.Concentration * s.DryNet * 0.001 * 1e-6) * (MyMath.NAvg * sigma.sigma0 * 1e-24 * sigma.theta * 0.01 / sigma.Mat) * (FC * 1e6 / 0.2882) * ir.R0 * MyMath.S((0.693 / n.T2), s.IrradiationTotalTime); //result in Bq
