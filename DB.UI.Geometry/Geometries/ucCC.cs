...
namespace DB.UI
{
    public partial class ucCC : UserControl
    {
        ...
        public void Set(ref Interface LinaaInterface)
        {
            ...
            // DataGridViewButtonColumn col = this.fluxTypeDGVColumn as DataGridViewButtonColumn; //
            // . Button;
        }
        ...
        private void ChannelDGV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (row.FluxType.Contains("0")) row.FluxType = Interface.IDB.MatSSFTYPES[1];
            else if (row.FluxType.Contains("1")) row.FluxType = Interface.IDB.MatSSFTYPES[2];
            else row.FluxType = Interface.IDB.MatSSFTYPES[0];
        }
