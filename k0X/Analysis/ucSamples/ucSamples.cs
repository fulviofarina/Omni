...
namespace k0X
{
    public partial class ucSamples : UserControl
    {
        public ucSamples(ref Interface set)
        {
            ...
            this.pTable = Program.UserControls.OfType<ucPeriodicTable>()
                      .FirstOrDefault();
        ...
        protected void Preferences(bool load)
        {
            try
            {
                ...
                if (load)
                {
                    ...
                    if (!EC.IsNuDelDetch(iSubS.Irradiation))
                    {
                        if (!EC.IsNuDelDetch(iSubS.Irradiation.ChannelsRow))
                        {
                            fluxTypebox.Text = iSubS.Irradiation.ChannelsRow.FluxType;
                        }
                    }
                }
                else
                {
                    ...
                    if (!EC.IsNuDelDetch(iSubS.Irradiation))
                    {
                        if (!EC.IsNuDelDetch(iSubS.Irradiation.ChannelsRow))
                        {
                            iSubS.Irradiation.ChannelsRow.FluxType = fluxTypebox.Text;
                        }
                    }
        ...
        protected void ButtonVisible(bool visible)
        {
            ...
            else
            {
                ...
                else if (!this.iSubS.Offline) color = Color.LemonChiffon;
        ...
        protected void FindSamples()
        {
            ...
            if (!this.iSubS.Project.Equals(project))
            {
                this.iSubS.Project = project;    //gather samples from these project (this.Name)
            }
            this.samples = null;
