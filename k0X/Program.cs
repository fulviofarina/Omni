...
namespace k0X
{
  internal static class Program
  {
    ...
    public static IEnumerable<Process> ProcessList
    {
      get
      {
    ...
    {
      IEnumerable<Process> processlist = Program.ProcessList.Where(p => p.ProcessName.Contains("k0X"));
      processlist = processlist.OrderByDescending(o => o.StartTime).Skip(1);
      if (processlist.Count() >= 1)
      {
        foreach (Process theprocess in processlist) theprocess.Kill();
      }
    }
