...
C-M  dilution cross sections are printed.
C-M
...
C*                    Max. No. of energy-groups     = MXG
      ...
      CHARACTER*80 FLIN,FLOU,FLIB,FLXS,FLSP,FLCF
      CHARACTER*66 CH66
...
C*
   68 ISRC=0
      IF(SMT.GT.0 .AND. SMD.GT.0) THEN
        ...
        WRITE(LTT,611) '$Enter ISRC,CHD,CHT (free format)     : '
   69   READ (LIN,612) LNI
        IF(LNI(1:2).EQ.'--') THEN
          WRITE(LTT,612) LNI
          GO TO 69
        END IF
        IF(LNI(1:40).EQ.BLN) GO TO 70
        READ (LNI, * ,ERR=70,END=70) SRC,CHD,CHT
        CHD=CHD/10
        CHT=CHT/10
        ISRC=NINT(SRC)
        IF(ISRC.LE.0) THEN
...
C* Mean chord length for a wire lying flat in cylindrical source
          H2R=CHT/CHD
          AH2R=ATAN(H2R)
...
C         CDL=PI*SMD*SMT/(2*PI*SMD+8*SMT))
        ELSE
C* Mean chord length for a cylindrical source of diameter CHD, height CHT
...
C* Absorption and elastic resonance integral
        ...
        DO J=2,NG
          ...
          END IF
        END DO
C* Tune the Bell factor depending on the ratio of elastic/total
        IF(ISRC.EQ.1) THEN
