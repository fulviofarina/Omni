...
namespace k0X
{
    public interface IWatchDog
    {
        ...
        void Link(ref DB.LINAA lims, string Project);
    }
    ...
    public partial class ucWatchDog : UserControl, IWatchDog
    {
        ...
        public void Link(ref DB.LINAA lims, string Project)
        {
            this.ParentForm.Text = "WatchDog - " + project;

            exceptionDT = lims.Exceptions;
            this.Linaa.QTA = lims.QTA;
            this.Linaa.CurrentPref = lims.CurrentPref;
            this.Linaa.Notify = lims.Notify;

            project = Project.ToUpper();
            irrId = lims.IrradiationRequests.FindIrrReqID(project);
            samples = lims.SubSamples.FindByIrReqID(irrId).ToList();
            meas = samples.SelectMany(s => s.GetMeasurementsRows()).ToList();

            Dirbox.Text = this.Linaa.CurrentPref.Spectra + project;
        ...
        private void watcher_Created(object sender, System.IO.FileSystemEventArgs e)
        {
            try
            {
                ...
                if (isDig)
                {
                    ...
                    IList<string> measurements = GetMeasurements(this.Linaa.CurrentPref.Spectra, project, sample);
